...
namespace
{
	struct dictionary
	{
		...
		std::pair<unsigned int, DTCId> unorderedMapUIntToDTC_data = std::make_pair<unsigned int, DTCId>(0,DTCId());
		std::pair<DTCId, unsigned int> unorderedMapDTCToUInt_data = std::make_pair<DTCId, unsigned int>(DTCId(),0);
	};
}
